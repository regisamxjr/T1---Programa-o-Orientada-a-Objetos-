...
public class CadastroPartido {
	...
	public void votosPrefeitoDeUmPartido(String nome) {
		Partido partido = consultaPartidoPorNome(nome);
		if (partido != null) {
			for (Candidato c : partido.getCandidato()) {
				if (c.getNumero() < 100) {
					System.out.println("6: " +partido.getNome() + c.getNumero() + c.getNome() + c.getMunicipio() + c.getVotos());
				} else {
